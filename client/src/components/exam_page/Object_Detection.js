...
export default class Detection extends React.Component {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      const webCamPromise = navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          if (this.props.setCameraStream) {
            this.props.setCameraStream(stream);
          }
  ...
  triggerAlert = (title, text) => {
    const now = Date.now();
    if (now - this.state.lastAlertTime > 2000) {
      this.setState({ lastAlertTime: now });
  ...
  renderPredictions = (predictions) => {
    ...
    predictions.forEach((prediction) => {
      ...
      if (["book", "laptop"].includes(prediction.class)) hasProhibited = true;
    });
    ...
    if (predictions.length === 0) {
      if (this.state.count < 50) {
        ...
        this.props.FaceNotVisible?.();
        this.triggerAlert("Face Not Visible", "Action has been recorded");
      }
    } else {
      this.setState({ count: 0 });
    }
    ...
    if (faces > 1) {
      this.props.MultipleFacesVisible?.();
      this.triggerAlert(`${faces} people detected`, "Action has been recorded");
    }

    if (hasPhone) {
      this.props.MobilePhone?.();
      this.triggerAlert("Cell Phone Detected", "Action has been recorded");
    }

    if (hasProhibited) {
      this.props.ProhibitedObject?.();
      this.triggerAlert("Prohibited Object Detected", "Action has been recorded");
    }
  };
